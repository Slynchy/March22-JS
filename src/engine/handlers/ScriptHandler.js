...
class ScriptHandler {
  ...
  Goto(checkpointName) {
    ...
    console.log(this._currentScript.checkpoints[found]);
  ...
  ExecuteFunction(line_c, isInline) {
    ...
    if (line_c.m_lineType === LineTypes.NARRATIVE) {
      if(this._novelMode){
        this._engine.SceneHandler.textBox.setTextbox('novel');
      } else {
        this._engine.SceneHandler.textBox.setTextbox('narrative');
      }
      ...
      this._engine.SceneHandler.textBox.setTextbox('dialogue');
      this._engine.SceneHandler.textBox.setText(line_c.m_lineContents);
